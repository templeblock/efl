...
static Eina_Bool _bridge_path_from_object(Eo *bridge, const Eo *eo, const char **path, const char **bus);
...
static void _elm_atspi_bridge_pending_add(Eo *obj, Eldbus_Pending *pending);
static void _elm_atspi_bridge_pending_del(Eo *obj, Eldbus_Pending *pending);
static void _elm_atspi_bridge_pending_cancel_all(Eo *obj);
...
static Eina_Bool
_bridge_path_from_object(Eo *bridge, const Eo *eo, const char **out_path, const char **out_bus)
{
   static char path[64];
   const char *bus;
   ...
   if (!pd->cache || !eina_hash_find(pd->cache, &eo))
     return EINA_FALSE;

   if (efl_isa(eo, ELM_ATSPI_PROXY_CLASS))
     {
        // what address should i return?
        // -> if address and bus is fixed, return fixed
        // -> if address is not fixed, it should return address of remotly set address
        elm_atspi_proxy_fixed_address_get((Elm_Atspi_Proxy*)eo, out_path, &bus);
     }
   else
     {
        if (eo == pd->root)
        ...
        bus = eldbus_connection_unique_name_get(pd->a11y_bus);
     }
...
_bridge_iter_object_reference_append(Eo *bridge, Eldbus_Message_Iter *iter, const Eo *obj)
{
   ...
   if (!_bridge_path_from_object(bridge, obj, &path, &bus))
     {
...
static void
_on_elm_atspi_bridge_app_register(void *data, const Eldbus_Message *msg, Eldbus_Pending *pending)
{
...
_elm_atspi_bridge_app_register(Eo *bridge)
{
   ...
   _bridge_iter_object_reference_append(bridge, iter, pd->root);
   register_req = eldbus_connection_send(pd->a11y_bus, message, _on_elm_atspi_bridge_app_register, bridge, -1);
   if (!register_req)
     {
...
static void _bridge_signal_send(Eo *bridge, Eo *obj, const char *infc, const Eldbus_Signal *signal, const char *minor, unsigned int det1, unsigned int det2, const char *variant_sig, ...)
{
             ...
             switch (*tmp)
               {
                ...
                case 'o':
                   ...
                   _bridge_path_from_object(bridge, atspi_obj, &path, &bus);
   ...
   iter_struct = eldbus_message_iter_container_new(iter, 'r', NULL);
   _bridge_path_from_object(bridge, pd->root, &path, &bus);
   eldbus_message_iter_basic_append(iter_struct, 's', bus);
   eldbus_message_iter_basic_append(iter_struct, 'o', path);
   eldbus_message_iter_container_close(iter, iter_struct);
...
{
   Elm_Atspi_Bridge *bridge = data;
   elm_atspi_bridge_disconnect(bridge);
}
...
_elm_atspi_bridge_connection_init(Elm_Atspi_Bridge *bridge, const char *address)
{
   ...
   if (pd->a11y_bus)
     {
        pd->connected = EINA_TRUE;
        elm_atspi_bridge_on_connected(bridge);
...
_elm_atspi_bridge_a11y_stack_enabled_get(void *data, const Eldbus_Message *msg, Eldbus_Pending *pending EINA_UNUSED)
{
   ...
   if (is_enabled)
      elm_atspi_bridge_connect(bridge);
   else
      elm_atspi_bridge_disconnect(bridge);
}
...
static Eina_Bool
_elm_atspi_bridge_a11y_stack_enabled_monitor_add(Elm_Atspi_Bridge *bridge)
{
   Eldbus_Proxy *proxy;
   Eldbus_Pending *req;
...
static void
_elm_atspi_bridge_a11y_stack_enabled_monitor_del(Elm_Atspi_Bridge *bridge)
{
   ELM_ATSPI_BRIDGE_DATA_GET_OR_RETURN(bridge, pd);
...
EOLIAN Efl_Object*
_elm_atspi_bridge_efl_object_constructor(Eo *obj, Elm_Atspi_Bridge_Data *pd EINA_UNUSED)
{
   efl_constructor(efl_super(obj, ELM_ATSPI_BRIDGE_CLASS));

   elm_need_eldbus();
   elm_atspi_bridge_auto_connect_set(obj, EINA_TRUE);
...
{
   elm_atspi_bridge_disconnect(obj);
   elm_atspi_bridge_root_set(obj, NULL);
   elm_atspi_bridge_auto_connect_set(obj, EINA_FALSE);
...
_elm_atspi_bridge_root_set(Eo *obj, Elm_Atspi_Bridge_Data *pd, Efl_Object *new_root)
{
   ...
   if (was_connected)
      elm_atspi_bridge_connect(obj);
}

EOLIAN static void
_elm_atspi_bridge_connect(Eo *bridge, Elm_Atspi_Bridge_Data *pd)
{
   ...
   pd->connect_request = eldbus_object_send(pd->bus_obj, m, _elm_atspi_bridge_a11y_bus_address_get, bridge, 100);
...
EOLIAN static void
_elm_atspi_bridge_disconnect(Eo *bridge, Elm_Atspi_Bridge_Data *pd)
{
   if (pd->connect_request)
     {
...
static void
_elm_atspi_bridge_pending_del(Eo *bridge, Eldbus_Pending *pending)
{
   ELM_ATSPI_BRIDGE_DATA_GET_OR_RETURN(bridge, pd);
...
static void
_elm_atspi_bridge_pending_cancel_all(Eo *bridge)
{
   ELM_ATSPI_BRIDGE_DATA_GET_OR_RETURN(bridge, pd);
...
EOLIAN void
_elm_atspi_bridge_on_disconnected(Eo *obj, Elm_Atspi_Bridge_Data *pd)
{
   _elm_atspi_bridge_pending_cancel_all(obj);

   if (pd->root) _elm_atspi_bridge_app_unregister(obj);
   _elm_atspi_bridge_event_handlers_unregister(obj);
   _elm_atspi_bridge_interfaces_unregister(obj);
...
EOLIAN void
_elm_atspi_bridge_on_connected(Eo *obj, Elm_Atspi_Bridge_Data *pd)
{
   pd->cache = eina_hash_pointer_new(NULL);
   pd->state_hash = _elm_atspi_state_hash_build();

   _elm_atspi_bridge_interfaces_register(obj);
   _elm_atspi_bridge_event_handlers_register(obj);

   if (pd->root) _elm_atspi_bridge_app_register(obj);
}

EOLIAN Eina_Bool
_elm_atspi_bridge_auto_connect_get(Eo *obj EINA_UNUSED, Elm_Atspi_Bridge_Data *pd)
{
   return pd->auto_connect;
}

EOLIAN void
_elm_atspi_bridge_auto_connect_set(Eo *obj, Elm_Atspi_Bridge_Data *pd, Eina_Bool val)
{
   if (pd->auto_connect == val)
     return;

   pd->auto_connect = val;

   if (val)
      _elm_atspi_bridge_a11y_stack_enabled_monitor_add(obj);
   else
      _elm_atspi_bridge_a11y_stack_enabled_monitor_del(obj);
}

EOLIAN Eldbus_Object*
_elm_atspi_bridge_eldbus_object_get(Eo *obj, Elm_Atspi_Bridge_Data *pd, Elm_Interface_Atspi_Accessible *accessible)
{
   const char *path, *bus;
   ...
   if (!_bridge_path_from_object(obj, accessible, &path, &bus))
     return NULL;

   return eldbus_object_get(pd->a11y_bus, bus, path);
}
